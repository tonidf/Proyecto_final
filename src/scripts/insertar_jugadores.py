...
#     cur.execute("SELECT id, api_id FROM equipos WHERE id >= 15")
...
def check_team(api_id):
    resp = requests.get(API_URL, headers=HEADERS, params={"id": api_id})
    data = resp.json()
    print(f"Team ID {api_id}: status {resp.status_code}, resultados={data.get('results')}")
    return data

for equipo_id, api_id in [(15, 715), (16, 723), (17, 724),
                        (18, 727), (19, 728), (20, 798)]:
    print("—" * 30)
    res = check_team(api_id)
    print(res)
    resp = requests.get(
    "https://v3.football.api-sports.io/players",
    headers=HEADERS,
    params={"team": api_id, "season": 2023}
)
    data = resp.json()
    print(f"Players for team {api_id}: status={resp.status_code}, results={data.get('paging', {}).get('total')}, jugadores en primera página={len(data.get('response', []))}")